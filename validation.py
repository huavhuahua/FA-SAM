...
parser.add_argument('-tdp', '--test_data_path', type=str, default='/home1/HWGroup/xrwu/Data/SAM_Med3D/test/BraTS2021_test_t1ce_label4')
parser.add_argument('-vp', '--vis_path', type=str, default='/home1/HWGroup/xrwu/Data/SAM_Med3D/test/BraTS2021_test_t1ce_label4/HUIFU_results_flavg')
parser.add_argument('-cp', '--checkpoint_path', type=str, default='work_dir/fl_train/t1ce_label3/sam_model_client4_dice_best.pth')
parser.add_argument('--save_name', type=str, default='./results/H-BraTS2021_test_t1ce_label4.py')
...
if __name__ == "__main__":    
    ...
    for batch_data in tqdm(test_dataloader):
        image3D, gt3D, padding_params, cropping_params, img_name = batch_data
        padding_params = [p.item() for p in padding_params]
        cropping_params = [c.item() for c in cropping_params]
        sz = image3D.size()
        ...
        if(os.path.exists(pred_path)):
            ...
            for idx, pred3D in enumerate(seg_mask_list):
                pred3D_tensor = torch.from_numpy(pred3D)
                pred3D_tensor = pred3D_tensor.unsqueeze(0)
                subject = tio.Subject(
                ...
                restored_output = np.transpose(restored_output, (0, 3, 1, 2))

                out = sitk.GetImageFromArray(restored_output[0])
